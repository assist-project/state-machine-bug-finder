...
**StateMachineBugFinder** (or **SMBugFinder** for short) is a cross-platform, automated bug detection framework whose functionality and usage are described in an [ISSTA'24 tool paper][issta24paper] that we recommend to cite if you want to refer to this tool.

In a nutshell, **SMbugFinder** implements the automata-based technique described in a [paper at NDSS 2023][ndss23paper].
It takes as input a state machine model of a System Under Test (SUT), e.g., a state machine model generated by a protocol state fuzzer for some protocol implementation, and a catalogue of bug patterns for the corresponding protocol.
It then uses this bug catalogue to automatically detect bugs in the SUT's state machine.

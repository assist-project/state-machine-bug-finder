...
/**
 * StateMachineBugFinder or SMBugFinder in short, is a library for automating
 * the detection of state machine bugs in SUTs. It takes as input the Mealy
 * machine model of the SUT and a catalog of bugs expressed as DFAs. It then
 * automatically detects the DFA-encoded bugs on the Mealy machine, generating
 * witnesses exposing the violations. The witnesses can optionally be validated
 * by executing the corresponding inputs on the SUT. This last step requires a
 * test harness.
 */
public class StateMachineBugFinder<I, O> {
    ...
    public BugReport<I, O> findBugs(BugPatterns patterns, MealyMachine<?, I, ?, O> mealy, Collection<I> inputs,
            SymbolMapping<I, O> mapping, @Nullable SUT<I, O> sut, List<StateMachineBug<I, O>> bugs) {
        ...
        if (validate) {
            ...
            InputCountingSUT<I, O> inputCountingSut = new InputCountingSUT<I, O>(resetCountingSut,
                    new Counter("inputs"));
        ...
        for (BugPattern bugPattern : patterns.getSpecificBugPatterns()) {
            ...
            if (bugLanguage.isEmpty()) {
                LOGGER.info(
                        "The bug pattern {} is an empty language when considering only the input/output labels from the SUT model. ",
                        bugPattern.getName());
                continue;
            }

            DFAAdapter sutBugLanguage = bugPattern.generateBugLanguage().intersect(sutLanguage).minimize();
            ...
            if (!sutBugLanguage.isEmpty()) {
                ...
                if (DebugMode.COUNT_GENERATED_WITNESSES.isEnabled(config)) {
                    ...
                    if (count < config.getDebugWitnessBound()) {
                        LOGGER.info(
                                "The bug pattern {} would lead to the generation of at most {} witnesses for validation.",
                                bugPattern.getName(), count);
                    } else {
                        LOGGER.info(
                                "The bug pattern {} would lead to the generation of over {} witnesses for validation.",
                ...
                if (validate && !DebugMode.EVALUATE_SPECIFIC_BUG_PATTERNS.isEnabled(config)) {
                    try {
                        SequenceGenerator<Symbol> sequenceGenerator = SequenceGeneratorFactory
                                .buildGenerator(config.getWitnessGenerationStrategy(), config.getSearchConfig(), null);
                        ...
                        Trace<I, O> witness = witnessFinder.findWitness(sut, mapping, sutBugLanguage,
                                bugPattern.generateBugLanguage());
                        ...
                        if (witness != null) {
                            ...
                            bugs.add(bug);
                            LOGGER.info("Could not find valid witness, giving counterexample {}",
                                    counterexample.toCompactString());
                        }
    ...
    private void handleGeneralBugPattern(GeneralBugPattern generalBugPattern, DFAAdapter sutLanguage,
            Collection<BugPattern> specificBugPatterns, SymbolMapping<I, O> mapping, List<StateMachineBug<I, O>> bugs) {
        ...
        List<BugPattern> specializedBugPatterns = specificBugPatterns.stream()
                .filter(sbp -> !sutBugLanguage.intersect(sbp.generateBugLanguage()).isEmpty())
                .collect(Collectors.toList());

        int uncategorizedSequences = 0, generatedSequences = 0;

        for (Word<Symbol> sequence : wordsToAcceptingStates(sutBugLanguage.getDfa(), sutBugLanguage.getSymbols(),
                search)) {
            generatedSequences++;
            List<BugPattern> capturingBps = specializedBugPatterns.stream()
                    .filter(bp -> bp.generateBugLanguage().accepts(sequence)).collect(Collectors.toList());
    ...
    private void evaluateSpecificBugPatterns(GeneralBugPattern generalBugPattern, DFAAdapter sutLanguage,
            Collection<BugPattern> detectedSpecificBugPatterns, SymbolMapping<I, O> mapping, @Nullable SUT<I, O> sut) {
        LOGGER.info("Using the general bug pattern {} to evaluate (the benefit of) the specific bug patterns",
                generalBugPattern.getShortenedName());
        DFAAdapter bugLanguage = generalBugPattern.generateBugLanguage().minimize();
        exporter.exportDfa(bugLanguage, generalBugPattern.getShortenedName() + "Language.dot");
        ...
        List<BugPattern> specificBps = detectedSpecificBugPatterns.stream()
                .filter(sbp -> !sutBugLanguage.intersect(sbp.generateBugLanguage()).isEmpty())
                .collect(Collectors.toList());
        ...
        int uncategorizedSequences = 0, generatedSequences = 0, validatedSequences = 0,
                validatedUncategorizedSequences = 0;

        for (Word<Symbol> sequence : wordsToAcceptingStates(sutBugLanguage.getDfa(), sutBugLanguage.getSymbols(),
                search)) {
            generatedSequences++;
            List<BugPattern> capturingBps = specificBps.stream()
                    .filter(bp -> bp.generateBugLanguage().acceptsPrefix(sequence)).collect(Collectors.toList());
            if (capturingBps.isEmpty()) {
                uncategorizedSequences++;
            }
        ...
        if (!uncategorizedSpecificBps.isEmpty()) {
            LOGGER.info("Specific bug patterns found in SUT that are not captured by the general bug pattern ({})",
                    uncategorizedSpecificBps.toString());
        } else {
            LOGGER.info("All bug patterns found in SUT are captured by the general bug pattern");
        }
        LOGGER.info("Specific bug patterns found in both general bug pattern and SUT ({}): {}", specificBps.size(),
                specificBps.toString());
        LOGGER.info("Specific bug patterns which were covered by sequence generation ({}): {}", coveredBps.size(),
                coveredBps.toString());
        if (coveredBps.size() != specificBps.size()) {
            Set<BugPattern> specificBpsNotCovered = new LinkedHashSet<>(specificBps);
            specificBpsNotCovered.removeAll(coveredBps);
            LOGGER.info("Specific bug patterns that were not covered by sequence generation ({}): {}",
                    specificBpsNotCovered.size(), specificBpsNotCovered.toString());
        } else {
        ...
        if (validate) {
            ...
            LOGGER.info("Validated categorizing bug patterns ({}): {}", validatedCategorizingBps.size(),
                    validatedCategorizingBps.toString());
            if (validatedCategorizingBps.size() != specificBps.size()) {
                Set<BugPattern> specificBpsNotCovered = new LinkedHashSet<>(specificBps);
                specificBpsNotCovered.removeAll(validatedCategorizingBps);
                LOGGER.info("Specific bug patterns that were not validated({}): {}", specificBpsNotCovered.size(),
                        specificBpsNotCovered.toString());
            } else {
                LOGGER.info("All specific bug patterns have been validated");
            }
            tracker.endValidation(generalBugPattern);
            tracker.handleValidatedGeneralBugPattern(generalBugPattern, validatedSequences,
                    validatedUncategorizedSequences);
        }
        tracker.handleGeneralBugPattern(generalBugPattern, generatedSequences, uncategorizedSequences);
    }

    private void handleUncategorizedSpecificationBugs(DFAAdapter specLanguage, DFAAdapter sutLanguage,
            Collection<BugPattern> bugPatterns, MealyMachine<?, I, ?, O> mealy, SymbolMapping<I, O> mapping,
            List<StateMachineBug<I, O>> bugs) {
        ...
        for (Word<Symbol> sequence : wordsToAcceptingStates(sutSpecBugLanguage.getDfa(),
                sutSpecBugLanguage.getSymbols(), search)) {
            allFlows++;
            List<AbstractBugPattern> capturingBps = bugPatterns.stream()
                    .filter(bp -> bp.generateBugLanguage().accepts(sequence)).collect(Collectors.toList());
            ...
            if (capturingBps.isEmpty()) {
                if (specLanguage.accepts(sequence)) {
                    throw new InternalError(
                            "Accepting sequence in uncategorized bug language accepted by specification");
                }
    ...
    private <S> Iterable<Word<Symbol>> wordsToAcceptingStates(DFA<S, Symbol> dfa, Collection<Symbol> symbols,
            SearchConfig search) {
    ...
    private <MS> Object getDeviantTransition(Word<Symbol> sequence, DFAAdapter spec, MealyMachine<MS, I, ?, O> mealy,
    ...
    private <S> void allowedSymbols(DFA<S, Symbol> specDfa, Word<Symbol> fromPrefix, Collection<Symbol> usedSymbols,
            Collection<Symbol> allowedSymbols) {
